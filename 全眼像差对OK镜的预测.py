...
# ## 建立模型
# 
# 随手建立的最简单模型，输入数据使用能找到的所有数据，这样可能噪声会很大。
# 
# 分成两类，
# 
# * 拟合模型，也就是通过一大堆的输入，争取能够猜出未来眼轴变化的准确数值。
# * 分类模型，设定一个阈值，比如眼轴增长是否>0.2mm，通过一堆数据输入，猜出未来眼轴是否大于阈值。
...
# 分类模型的评价指标除了R-squared的得分，更好的是使用ROC曲线，以及曲线下的面积AUC。
# 
# * ROC曲线越偏向左上方，AUC越接近1，模型越好
# * ROC曲线接近0.5的红线，AUC评分接近0.5，则接近于瞎猜。
